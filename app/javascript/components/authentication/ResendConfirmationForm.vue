    ...
    >
      ...
      <v-field>
        <button
          ...
          type="submit"
        >
          {{ $t('devise.ctas.resend_confirmation_button') }}
        </button>
      </v-field>
    </form>
  </div>
...
export default {
  ...
  methods: {
    onSubmit () {
      ...
      this.$http.post(`/${this.$i18n.locale}/users/confirmation`, { user: this.form })
        .then(this.onSuccess)
        .catch(this.onError)
        .finally(() => {
          this.busy = false
    ...
    onSuccess () {
      window.open(`/${this.$i18n.locale}/users/sign_in`, '_self')
    },
    onError (e) {
