...
def dockerImageCleanup() {
    ...
    sh "docker-compose --project-name=${JOB_NAME} down --volumes --remove-orphans"
sh """docker ps -a --no-trunc  | grep "${BRANCH_NAME}" | awk '{print $1}' | xargs -r --no-run-if-empty docker stop && \
docker ps -a --no-trunc  | grep "${BRANCH_NAME}" | awk '{print $1}' | xargs -r --no-run-if-empty docker rm && \
docker images --no-trunc | grep "${BRANCH_NAME}" | awk '{print $3}' | xargs -r --no-run-if-empty docker rmi"
"""
