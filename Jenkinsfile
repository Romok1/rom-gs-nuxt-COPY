...
def dockerImageCleanup() {
    ...
    sh '''#!/bin/bash
	docker ps -a --no-trunc  | grep "^${JOB_BASE_NAME}" | awk '{print $1}' | xargs -r --no-run-if-empty docker stop -f
	docker ps -a --no-trunc  | grep "^${JOB_BASE_NAME}" | awk '{print $1}' | xargs -r --no-run-if-empty docker rm -f
	docker images --no-trunc | grep "^${JOB_BASE_NAME}" | awk '{print $3}' | xargs -r --no-run-if-empty docker rmi -f
    '''
