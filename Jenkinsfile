...
def dockerImageCleanup() {
    ...
    sh "docker ps -a --no-trunc  | grep 'test' | awk '{print $1}' | xargs -r --no-run-if-empty docker stop"
	
	
sh """docker ps -a --no-trunc  | grep '$test' | awk '{print $1}' | xargs -r --no-run-if-empty docker rm
docker images --no-trunc | grep 'test' | awk '{print $3}' | xargs -r --no-run-if-empty docker rmi
"""
