pipeline {
    ...
    stages {
		...
		severity: 'critical', 
		additionalArguments: '--all-projects --detection-depth=4 --policy-path=nuxt-frontend --targetFiles=rails-api/Gemfile, --target-dir=rails-api --configuration-matching=^(?!Gemfile).* --prune-repeated-subdependencies --debug',
              )
