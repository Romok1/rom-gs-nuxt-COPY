pipeline {
    ...
    stages {
		...
		severity: 'critical', 
		additionalArguments: '--all-projects --detection-depth=4 --exclude=package.json, --target-dir=rails-api --configuration-matching=^(?!Gemfile).* --prune-repeated-subdependencies --debug',
              )
            }
	   post {
                  success{
