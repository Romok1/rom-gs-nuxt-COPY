...
def dockerImageCleanup() {
	...
	sh """docker ps -a --no-trunc  | grep "get-app-test" | awk '{print $1}' | xargs -r --no-run-if-empty docker stop && \
docker ps -a --no-trunc  | grep "get-app-test" | awk '{print $1}' | xargs -r --no-run-if-empty docker rm && \
docker images --no-trunc | grep "get-app-test" | awk '{print $3}' | xargs -r --no-run-if-empty docker rmi"
"""
